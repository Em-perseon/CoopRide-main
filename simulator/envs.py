...
class CityReal:
    ...
    def __init__(self, mapped_matrix_int, order_num_dist, idle_driver_dist_time, idle_driver_location_mat, order_time_dist, order_price_dist,
                 ...
                 reward_weights=None, reward_gmv_scale=1.0):
        """
        ...
        dispatch_limit = os.environ.get("COOPRIDE_DISPATCH_LIMIT", "")
        try:
            self.dispatch_limit = int(dispatch_limit) if dispatch_limit else None
        except ValueError:
            self.dispatch_limit = None
        ...
        self.reward_gmv_scale = float(reward_gmv_scale)
        self.node_reward_cache = None
        ...
        self.global_instruction_vector = None
        self.global_instruction_dim = None
        ...
        if self.neighbor_dis is None:
            self.neighbor_dis = np.zeros((self.n_nodes,self.n_nodes),dtype=np.int32)+100
            for i in range(self.n_nodes):
                self.neighbor_dis[i,i]=0
                for j in range(9):
                    nei = self.layer_neighborhood[i][j]
                    self.neighbor_dis[i,nei] = j+1
    ...
    def set_reward_weights(self, weights):
        ...
        for key in ("gmv", "orr"):
            if key in weights:
                self.reward_weights[key] = float(weights[key])
        if "gmv_scale" in weights:
            self.reward_gmv_scale = float(weights["gmv_scale"])

    def compute_reward(self, gmv_score, orr_score):
        return (
            self.reward_weights.get("gmv", 1.0) * gmv_score
            + self.reward_weights.get("orr", 1.0) * orr_score
        )

    def get_node_reward_vector(self):
        rewards = []
        cache = self.node_reward_cache
        for idx, node in enumerate(self.nodes):
            if node is None:
                continue
            if cache is None:
                rewards.append(node.gmv)
            else:
                rewards.append(cache[idx])
        return np.array(rewards, dtype=np.float32)

    def get_state_space_node(self):
        if self.state_space == None:
            base_dim = 5 + self.l_max * 3
            if self.global_instruction_dim:
                base_dim += int(self.global_instruction_dim)
            self.state_space = base_dim
    ...
    def construct_mapping(self):
        ...
        :return:
        """
        # Include grid id 0 as valid (>=0); otherwise orders to/from grid 0
        # will be treated as out-of-grid and cause None end positions.
        ...
        for _node in self.nodes:
            if _node is not None:
                if _node.real_order_num > 0:
                    ...
                    if self.global_instruction_vector is not None:
                        _state.extend(self.global_instruction_vector.tolist())
    ...
    def set_global_instruction_vector(self, vector: Optional[np.ndarray]):
        if vector is None:
            self.global_instruction_vector = None
            return
        vec = np.asarray(vector, dtype=np.float32).reshape(-1)
        if self.global_instruction_dim is None:
            self.global_instruction_dim = int(vec.size)
        if vec.size < self.global_instruction_dim:
            vec = np.pad(vec, (0, self.global_instruction_dim - vec.size), mode='constant')
        elif vec.size > self.global_instruction_dim:
            vec = vec[:self.global_instruction_dim]
        self.global_instruction_vector = vec
    ...
    def get_order_list_PPO(self):
        ...
        for i, _node in enumerate(self.nodes):
            if _node is not None:
                ...
                prefix = []
                start = 0
                if len(states) > 0 and states[0][4] >= 0:
                    prefix_state = states[0] + [feature[0][0]]
                    prefix.append((prefix_state, idx[0]))
                    start = 1
                for j in range(start, len(states)):
                    state = states[j] + [feature[j][0]]
                    order_pairs.append((state, idx[j]))
                order_pairs.sort(key=lambda x: x[0][2] if len(x[0]) > 2 else 0.0, reverse=True)
                ordered = prefix + order_pairs
                order_state.append([pair[0] for pair in ordered])
                order_idx.append([pair[1] for pair in ordered])
    ...
    def reset(self, mode='MDP', generate_order=1):
        """ Return initial observation: get order distribution and idle driver distribution
        
        Args:
            mode: 'MDP', 'PPO', 'PPO2' etc.
            generate_order: 1=use sampled orders, 0=use real orders from day_orders
        """
        ...
        # Use real orders or sampled orders based on generate_order parameter
        if generate_order == 0:
            # Use real orders from day_orders
            if len(self.day_orders) == 0 and len(self.real_orders) > 0:
                self.utility_bootstrap_oneday_order()
            moment = self.city_time % self.n_intervals
            if len(self.day_orders) > moment:
                self.step_bootstrap_order_real(self.day_orders[moment])
            else:
                # Fallback to sampled mode
    ...
    def utility_bootstrap_oneday_order(self):
        ...
        for iorder in one_day_orders:
            #  iorder: [origin, dest, start_time, duration, price, (optional) serve_type]
            start_id = int(iorder[0])
            end_id = int(iorder[1])
            start_time = int(iorder[2])
            if start_id not in self.node_mapping.keys() and end_id not in self.node_mapping.keys():
                continue
            start_node = self.node_mapping.get(start_id, -100)
            end_node = self.node_mapping.get(end_id, -100)
    ...
    def step_bootstrap_order_real(self, day_orders_t):
        ...
        for iorder in day_orders_t:
            ...
            if end_node_id in self.target_grids:
                ...
                end_node = None
            # Order.get_price applies a 0.1 scale; pre-scale to keep real prices.
            real_price = iorder[4] * 10
            start_node.add_order_real(
                self.city_time, end_node, iorder[3], real_price)
    ...
    def step_assign_order_broadcast_neighbor_reward_update(self, dispatch_action, neighbor_help=False,mode='MDP'):
        ...
        node_reward_cache = np.zeros(len(self.nodes), dtype=np.float32)
        for node_id,_orders in enumerate(dispatch_action):
            ...
            if _orders:
                ...
                for _order in _orders:
                    order_identity = id(_order)
                    if order_identity in seen:
                        continue
                    seen.add(order_identity)
                    dedup_orders.append(_order)
            else:
                dedup_orders = _orders
            if self.dispatch_limit and dedup_orders:
                dedup_orders = dedup_orders[: self.dispatch_limit]
            node_rewards, all_order_num_node, finished_real_num_node, servers, fake_fin_node, fleet_fin_node, all_fake_node, all_fleet_node, idle_num_node, fleet_num_node = node.simple_order_assign_real_sample(dedup_orders, self.city_time, self, self.fleet_help , self.neighbor_dispatch)
            #else:
            ...
            node_gmv = float(np.sum(node_rewards))
            self.gmv += node_gmv
            #all_order_num += all_order_num_node
            ...
            node_orr = float(finished_real_num_node) / max(1.0, float(all_order_num_node))
            gmv_score = node_gmv * 0.01
            node_reward_cache[node_id] = self.compute_reward(gmv_score, node_orr)
        ...
        #else:
        #    self.order_response_rate = -1
        self.node_reward_cache = node_reward_cache
