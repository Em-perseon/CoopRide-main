...
class Environment():
    ...
    def __init__(self, mapped_matrix_int, order_num_dist, idle_driver_dist_time, idle_driver_location_mat, order_time_dist, order_price_dist,
                 ...
                 global_flag="global", time_interval=10, fleet_rate=1, fleet_help=False,grid_neighbor=None):
        """
        ...
        for _node in self.nodes:
            if _node is not None:
                if _node.real_order_num > 0:
                    # row_id, column_id = ids_1dto2d(_node.get_node_index(), self.M, self.N)
                    _state = [self.city_time,
                              _node.get_node_index(),  # for split
    ...
    def compute_metric(self):
        dist = self.step_get_distribution()
        entr_value = self.step_get_entropy()
        ...
        return self.gmv, self.order_response_rate

    def get_neighbors(self):
        neighbors = [node.layers_neighbors_id for node in self.nodes]
        return neighbors

    def get_reward(self):
        return torch.Tensor([ node.gmv for node in self.nodes] ) 

    def step(self, action_ids, generate_order=1, mode='SAC'):
        ...
        #self.check_nodewise()
        dispatch_actions = self.get_orders_by_id(action_ids)
...
class ParallelEnvironment():
    def __init__(self, params, args, device ):
        ray.init(num_cpus=args.cpu_num)
        self.envs = [Environment.remote(*params) for i in range(args.num_workers)]
        self.workers = args.num_workers
        self.device = device
        self.neighbors = ray.get(self.envs[0].get_neighbors.remote())

    def reset_randomseed(self,RANDOM_SEED):
        ray.get([env.reset_randomseed.remote(seed) for env, seed in zip(self.envs, RANDOM_SEED)])

    def state_space(self):
        return ray.get(self.envs[0].get_state_space_node.remote())

    def observe(self):
        return [torch.tensor(ray.get(i.observe.remote()), dtype=torch.float, device=self.device) for i in self.envs]

    def get_reward(self):
        reward = ray.get([i.get_reward.remote() for i in self.envs])
        return reward

    def reset(self,mode='PPO2'):
        state = ray.get([i.reset.remote(mode) for i in self.envs])
        state = list(zip(*state))
        # states_node, _, order_states, order_idx, order_feature, global_order_states
        state_node, state_order, order_idx = state[0], state[2], state[3]
        return state_node, state_order, order_idx

    def step(self, action,generate_order=1, mode='PPO2'):
        state = ray.get([env.step.remote(act,generate_order=generate_order,mode=mode) for env, act in zip(self.envs, action)])
        state_node,  state_order, order_idx, order_feature = list(zip(*state))
        return state_node, state_order, order_idx, order_feature

    def compute_metric(self):
        metric = ray.get([i.compute_metric.remote() for i in self.envs])
