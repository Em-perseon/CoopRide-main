...
class Node(object):
    ...
    def add_order_real(self, city_time, destination_node, duration, price):
        # current_node_id = self.get_node_index()
        # service_type=-1 表示真实订单，与 generate_order_real 一致
        # 必须是 -1，否则在 simple_order_assign_real_sample 中会被跳过
        self.orders.append(Order(self,
                                 destination_node,
                                 city_time,
                                 duration,
                                 price, 0, service_type=-1))
        self.order_num += 1
        self.real_order_num += 1  # 修复：同时更新 real_order_num
        self.new_order += 1  # 修复：更新 new_order 以正确计算 ORR
