...
def get_parameter():
    ...
    # parameter
    args.MAX_ITER=6000
    max_iter_env = os.environ.get("COOPRIDE_MAX_ITER")
    if max_iter_env:
        try:
            args.MAX_ITER = int(max_iter_env)
        except ValueError:
            pass
    args.model_save_interval = 10
    save_interval_env = os.environ.get("COOPRIDE_SAVE_INTERVAL")
    if save_interval_env:
        try:
            args.model_save_interval = int(save_interval_env)
        except ValueError:
            pass
    ...
    args.test = False
    test_env = os.environ.get("COOPRIDE_TEST", "0")
    if test_env == "1":
        args.test = True
    model_path_env = os.environ.get("COOPRIDE_MODEL_PATH")
    if model_path_env:
        args.model_dir = model_path_env
    args.TEST_ITER=1
    ...
    grid_num_env = os.environ.get("COOPRIDE_GRID_NUM")
    if grid_num_env:
        try:
            args.grid_num = int(grid_num_env)
        except ValueError:
            pass
    driver_dict = {
        143:2000,
    ...
    args.driver_num=driver_dict.get(args.grid_num, 2000)        # 改变初始化司机数量
    driver_num_env = os.environ.get("COOPRIDE_DRIVER_NUM")
    if driver_num_env:
        try:
            args.driver_num = int(driver_num_env)
        except ValueError:
            pass
    ...
    args.env_seed = seed_dict.get(args.grid_num, 0)
    args.custom_data_path = os.environ.get("COOPRIDE_DATA_PATH")
    
    # 是否使用真实订单数据（包含真实价格）
    use_real_orders_env = os.environ.get("COOPRIDE_USE_REAL_ORDERS", "0")
    args.use_real_orders = use_real_orders_env == "1"
    
    # 真实订单采样率 (0.0-1.0)，默认 0.1 (10%) 以提高训练速度
    sample_rate_env = os.environ.get("COOPRIDE_REAL_ORDER_SAMPLE_RATE", "0.1")
    try:
        args.real_order_sample_rate = float(sample_rate_env)
    except ValueError:
        args.real_order_sample_rate = 0.1
    ...
    disable_norm_env = os.environ.get("COOPRIDE_DISABLE_NORM", "0")
    if disable_norm_env == "1":
        args.use_valuenorm = False
        args.adv_normal = False
    ...
    feature_norm_env = os.environ.get("COOPRIDE_FEATURE_NORMAL")
    if feature_norm_env:
        try:
            args.feature_normal = int(feature_norm_env)
        except ValueError:
            pass
...
def _maybe_update_global_instruction(env, step):
    instructor = getattr(env, "llm_instructor", None)
    if instructor is None:
        return
    if getattr(env, "llm_dynamic_prompt", False):
        dispatch_interval = getattr(env, "llm_dispatch_interval", 10)
        try:
            dispatch_interval = int(dispatch_interval)
        except (TypeError, ValueError):
            dispatch_interval = 10
        hour = int((int(step) * dispatch_interval) // 60) % 24
        if hour in {8, 9, 17, 18, 19}:
            instructor.override_text = "High demand rush hour. Maximize efficiency."
            if hasattr(env, "set_reward_weights"):
                env.set_reward_weights({"gmv": 1.0, "orr": 0.0})
        else:
            instructor.override_text = "Standard operating conditions."
            if hasattr(env, "set_reward_weights"):
                env.set_reward_weights({"gmv": 0.0, "orr": 5.0})
    if getattr(env, "llm_contrastive", False):
        interval = getattr(env, "llm_contrastive_interval", 1)
        try:
        ...
        if interval <= 0:
            interval = 1
        last_step = getattr(env, "llm_last_mode_step", None)
        if last_step is None or (int(step) - int(last_step)) >= interval:
            env.llm_last_mode = "rush" if random.random() < 0.5 else "normal"
            env.llm_last_mode_step = int(step)
        mode = getattr(env, "llm_last_mode", "normal")
        if mode == "rush":
            instructor.override_text = "High demand surge. Prioritize high-value orders."
            if hasattr(env, "set_reward_weights"):
                env.set_reward_weights({"gmv": 1.0, "orr": 0.0})
        else:
            instructor.override_text = "Standard operation. Maximize Order Response Rate."
            if hasattr(env, "set_reward_weights"):
                env.set_reward_weights({"gmv": 0.0, "orr": 5.0})
    update_steps = getattr(env, "llm_update_steps", 1)
    if getattr(env, "llm_dynamic_prompt", False):
        update_steps = 1
    if getattr(env, "llm_contrastive", False):
        update_steps = 1
    ...
    if step % update_steps != 0:
        return
    if getattr(env, "_llm_last_update_step", None) == step:
        return
    try:
        env.set_global_instruction_vector(instructor.get_instruction_vector(env, step))
        env._llm_last_update_step = step
    except Exception as exc:
        print(f"[WARN] LLM instruction update failed: {exc}")
...
def train(env, agent , writer=None,args=None,device='cpu'):
    ...
    # Determine if using real orders or sampled orders
    use_real = getattr(args, 'use_real_orders', False)
    gen_order = 0 if use_real else 1
    ...
    for iteration in np.arange(args.resume_iter,args.MAX_ITER):
        ...
        _maybe_update_global_instruction(env, 0)
        states_node, _, order_states, order_idx, order_feature, global_order_states = env.reset(mode='PPO2', generate_order=gen_order)
        ...
        for T in np.arange(args.TIME_LEN):
            _maybe_update_global_instruction(env, T)
            ...
            next_states_node,  next_order_states, next_order_idx, next_order_feature= env.step(orders, generate_order=gen_order, mode='PPO2')
            ...
            if hasattr(env, "get_node_reward_vector"):
                reward = torch.Tensor(env.get_node_reward_vector())
            else:
        ...
        if args.model_save_interval > 0 and (iteration + 1) % args.model_save_interval == 0:
...
if __name__ == "__main__":
    ...
    if args.custom_data_path:
        env, M, N, _, args.grid_num = load_envs_custom(
            args.custom_data_path, 
            driver_num=args.driver_num,
            use_real_orders=args.use_real_orders,
            real_order_sample_rate=args.real_order_sample_rate
        )
        if args.use_real_orders:
            print(f"[INFO] 使用真实订单模式 (generate_order=0, sample_rate={args.real_order_sample_rate})")
    elif args.grid_num==100:
    ...
    # Update log_dir if grid_num changed (custom data path case)
    if args.custom_data_path:
        log_dir = '../logs/' + 'synthetic/'+'grid'+str(args.grid_num)+'/'+args.log_name.replace('OD143', f'OD{args.grid_num}')
        if os.path.exists(log_dir):
            log_dir+='_'+time.strftime("%Y%m%d_%H-%M")
        args.log_dir=log_dir
        mkdir_p(log_dir)
        print(f"[Updated] log dir is {log_dir}")
    
    env.fleet_help= args.FM_mode != 'local'

    if GlobalInstructionGenerator is not None and os.environ.get("COOPRIDE_LLM_ENABLED", "0") == "1":
        llm_instructor = GlobalInstructionGenerator.from_env()
        if llm_instructor.enabled:
            env.llm_instructor = llm_instructor
            env.llm_update_steps = llm_instructor.update_steps
            env.llm_dynamic_prompt = os.environ.get("COOPRIDE_LLM_DYNAMIC", "0") == "1"
            env.llm_contrastive = os.environ.get("COOPRIDE_LLM_CONTRASTIVE", "0") == "1"
            try:
                env.llm_contrastive_interval = int(os.environ.get("COOPRIDE_LLM_CONTRASTIVE_INTERVAL", "12"))
            except ValueError:
                env.llm_contrastive_interval = 12
            env.llm_dispatch_interval = args.dispatch_interval
            env._llm_last_update_step = None
            _maybe_update_global_instruction(env, 0)
    ...
    resume_path = os.environ.get("COOPRIDE_RESUME_PATH")
    if resume_path:
        print(f"Resuming from checkpoint: {resume_path}")
        agent.load_param(resume_path)
        resume_iter_env = os.environ.get("COOPRIDE_RESUME_ITER")
        if resume_iter_env:
            try:
                args.resume_iter = int(resume_iter_env)
            except ValueError:
                pass
